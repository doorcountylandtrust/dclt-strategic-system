# Rewritten Content System: How We Save Land Section

This document contains the full strategic rewrites for the "How We Save Land" section of the Door County Land Trust website. It includes a page-by-page restructuring using modular content blocks, refined calls-to-action, and updated voice and tone guidelines to align with the organizational mission. This version integrates insights from internal staff interviews.

---

## Table of Contents

1. [Overview: How We Save Land](#overview-how-we-save-land)
2. [Nature Preserves](#nature-preserves)
3. [Conservation Easements](#conservation-easements)
4. [Land Donations & Sales](#land-donations--sales)
5. [Partner Transfers](#partner-transfers)
6. [Landowner Inquiry Hub](#landowner-inquiry-hub)
7. [Caring for the Land](#caring-for-the-land)
8. [Research & Partnerships](#research--partnerships)
9. [Hunting Program](#hunting-program)
10. [Resources & Links](#resources--links)

---

## Overview: How We Save Land

**Purpose:** Introduce DCLT's core land protection methods in a digestible, inspiring format.

### Content Blocks:

* **Hero Banner**: Inspiring imagery of Door County's natural beauty
* **Intro Text**: "Land protection happens in many forms — and each one leaves a lasting legacy."
* **Three Pillars (Icon Grid):**

  * **Nature Preserves** (DCLT-owned)
  * **Conservation Easements** (privately owned, permanently protected)
  * **Partner Transfers** (lands DCLT protects and transfers to others)
* **CTA**: "Choose Your Path" → Volunteer | Donate | Protect Your Land

---

## Nature Preserves

**Purpose:** Explain what preserves are, highlight accessibility, and connect to Preserve Explorer.

### Content Blocks:

* **Definition**: "Public lands permanently protected and owned by DCLT"
* **Map Preview** or Carousel of Preserve Photos
* **Benefits**:

  * Public access
  * Habitat protection
  * Ecosystem restoration
  * *Preserves serve as ambassadors of our work* (staff insight)
  * *A visible entry point for people unfamiliar with conservation* (staff insight)
* **Volunteer Support**:

  * Trail maintenance and signage require ongoing stewardship
  * CTA: "Help us keep preserves safe and welcoming"
* **CTA**: Visit a Preserve | Donate | Volunteer
* **Crosslink**: Preserve Explorer map | Events at Preserves

---

## Conservation Easements

**Purpose:** Empower landowners to protect their land in perpetuity without giving up ownership.

### Content Blocks:

* **What is a Conservation Easement?** Plain-language definition
* **How it Works**:

  * Visual steps or FAQ accordion
  * *Relationship-first approach*: most easements develop over multiple years of trust-building (staff insight)
* **Benefits**:

  * Tax benefits
  * Legacy protection
  * Flexibility
* **Myths vs Facts** Block:

  * "I lose control of my land" → False
  * "It’s too complicated" → We handle the legal and ecological details
* **Testimonial**: Quote from local landowner
* **CTA**: Protect Your Land (Inquiry form)

---

## Land Donations & Sales

**Purpose:** Guide landowners through gift or sale options and highlight legacy potential.

### Content Blocks:

* **Ways to Give Land**:

  * Full donation
  * Bargain sale
  * Bequest in will
* **Tax Benefits** Overview
* **Decision Tree** or Scenario Examples
* **Quote or Legacy Story**
* **CTA**: Inquire About Donating Land

---

## Partner Transfers

**Purpose:** Showcase conservation outcomes even when DCLT does not retain ownership.

### Content Blocks:

* **Definition**: Land protected then transferred to municipalities, parks, or other nonprofits
* **Notable Examples**: Grand View, Schwartz Lake, etc.
* **Benefits**:

  * Larger scale protection
  * Community impact
* **CTA**: Learn More | Donate to Future Projects

---

## Landowner Inquiry Hub

**Purpose:** Single portal for interested landowners to ask questions and start the process.

### Content Blocks:

* **Intro Text**: “Your land matters. Let’s protect it together.”
* **Inquiry Form**
* **Map of Conservation Areas** (optional or link to Greenprint)
* **FAQ Accordion**
* **CTA**: Submit Your Property

---

## Caring for the Land

**Purpose:** Explain what stewardship means and how people can get involved in hands-on conservation.

### Content Blocks:

* **Stewardship Defined**: "Conservation doesn't stop once land is protected."
* **Key Activities**:

  * Forest restoration
  * Invasive species removal
  * Trail maintenance
  * *Seasonal cycles*: Winter planning and data work are just as critical (staff insight)
  * *Blend of science and sweat*: real ecological impact from volunteer work (staff insight)
* **Volunteer Highlight**:

  * Volunteers are trained and empowered, not just "extra hands"
* **CTA**: Join the Stew Crew

---

## Research & Partnerships

**Purpose:** Showcase DCLT’s role in scientific research and collaboration with partner orgs.

### Content Blocks:

* **Current Projects**:

  * Pollinator research
  * Pitcher’s Thistle
  * Wetland studies
* **How We Use This Research**:

  * Field data directly informs our restoration decisions (staff insight)
* **Partners**:

  * USFWS, WI-DNR, university researchers
  * Display logos or names
* **CTA**: Learn | Support | Volunteer

---

## Hunting Program

**Purpose:** Balance conservation with ethical hunting practices; provide clarity and safety.

### Content Blocks:

* **Intro Paragraph**: Hunters are conservation partners
* **Hunting Rules Summary**
* **Preserve Table**: Linked or downloadable
* **Hiker Safety Tips**
* **CTA**: Apply to Hunt | Donate | Contact Us

---

## Resources & Links

**Purpose:** Act as a library hub for maps, downloads, partner links, and environmental learning.

### Content Blocks:

* **Reorganized by Use Case**:

  * For Landowners
  * For Stewards
  * For Researchers
  * For Students
* **Downloadables**:

  * Maps
  * PDFs
  * Brochures
  * Volunteer log
* **Staff Picks**:

  * Include frequently referenced tools like groundwater maps or native plant guides
* **CTA**: Contact Us | Explore More

---

## Modular Add-ons (Apply Across Pages)

* **Story Spotlight Block**: Legacy at Clay Banks, Pitcher’s Thistle, etc.
* **Downloadable Forms**: Volunteer log, landowner packet
* **Quote Strip**: Jens Jensen or other mission-aligned thinkers
* **CTA Footer Block**: Join | Donate | Volunteer | Inquire

---

**Note:** This content system reflects both external audience needs and internal expertise gathered through staff interviews. It is ready for visual design exploration and implementation into the CMS block system.
